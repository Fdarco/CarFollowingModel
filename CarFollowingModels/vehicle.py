...
class vehicle:
    ...
    def DSMStep(self):
        ...
        accetau = self.accelerationCst(tempAcceleration)
        # print('{}: accetau = {}'.format(self.id, accetau))
        self.acceSeq.put(accetau)
    

    def FVDStep(self):
        if self.precedVeh:
            tempAcceleration = self.CFModel.getAcceleration(self, self.precedVeh)
        else:
            tempAcceleration = 0
